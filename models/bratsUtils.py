...
def bratsDiceLoss(outputs, labels, nonSquared=False):
    ...
    print("in bratsDiceLoss: ", outputs.shape)
    print("in bratsDiceLoss: ", labels.shape)
    # if '2D' dim=2 , if '3D' dim =1
    wt, tc, et = outputs.chunk(3, dim=1)
    print("wt.shape:", wt.shape)  
    print("tc.shape:", tc.shape)
    print("et.shape:", et.shape)
    s = wt.shape # s: torch.Size([4, 1, 128, 128])
    print("s:", s)
    ...
    #calculate losses
    print("wt.shape: ", wt.shape)
    print("wtMask.shape: ", wtMask.shape)
