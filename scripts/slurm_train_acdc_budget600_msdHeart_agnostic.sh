...
for lr in 0.01
   do
   for budget in 152 608
       do
       $exec_command python3 -u $code_path/run.py --exp-name "2021-08-01-train_acdc_MSDPretrained_ImageNetBackbone_128128_agnostic_budget${budget}_lr_${lr}_seed_${SLURM_ARRAY_TASK_ID}" \
...
for lr in 0.01
   ...
   for budget in 152 608
       do
       $exec_command python3 -u $code_path/run.py --exp-name "2021-08-01-train_acdc_MSDPretrained_ImageNetBackbone_256256_agnostic_budget${budget}_lr_${lr}_seed_${SLURM_ARRAY_TASK_ID}" \
       --seed ${SLURM_ARRAY_TASK_ID}  --checkpointer \
       --load-weights --exp-name-toload '2021-07-18-supervised_msdHeart0.01' \
       --ckpt-path $ckpt_path --data-path $data_path \
