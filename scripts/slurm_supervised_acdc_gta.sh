...
### Pretraining for ACDC ###
    for lr_rate in 0.005 0.01
        do
        $exec_command python3 $code_path/train_supervised.py --exp-name "2021-07-15-gta_pretraining_acdc_lr_${lr_rate}" --checkpointer \
        --ckpt-path $ckpt_path --data-path $data_path \
        --full-res --input-size 256 256 --dataset 'gta_for_acdc' \
