...
for budget in 112
    do
    $exec_command python3 -u $code_path/run.py --exp-name "2021-08-27-train_acdc_budget_${budget}_lr0.01_2patients_1stsetting" --seed ${SLURM_ARRAY_TASK_ID}  --checkpointer \
    --ckpt-path $ckpt_path --data-path $data_path  \
    --load-weights --exp-name-toload '2021-08-19-supervised_ImageNetBackbone_msdHeart234' --input-size 128 128 \
    --dataset 'acdc' --lr 0.01 --train-batch-size 32 --val-batch-size 8 --patience 30 --region-size 64 64 --epoch-num 10 --rl-episodes 100 --rl-buffer 600 --lr-dqn 0.001 --dqn-epochs 9 \
...
# for budget in 112
#     do
#     $exec_command python3 -u $code_path/run.py --exp-name "2021-08-26-3-train_acdc_ImageNetBackbone_budget_${budget}_lr0.01_2patients_3rdsetting" \
#     --seed ${SLURM_ARRAY_TASK_ID} --checkpointer --ckpt-path $ckpt_path --data-path $data_path \
#     --input-size 128 128 --only-last-labeled --dataset 'acdc' --lr 0.01 --train-batch-size 16 --val-batch-size 4 \
#     --patience 30 --region-size 64 64 --epoch-num 10 --rl-episodes 100 --rl-buffer 600 --lr-dqn 0.001 --dqn-epochs 10 \
#     --budget-labels $budget --num-each-iter 16 --al-algorithm 'ralis' --rl-pool 16 --only-last-labeled
...
# $exec_command python3 -u $code_path/run.py --exp-name "2021-08-23-train_acdc_budget_${budget}_lr0.01_2patients_newAug_1stsetting" --seed ${SLURM_ARRAY_TASK_ID}  --checkpointer 
# --ckpt-path $ckpt_path --data-path $data_path  --load-weights --exp-name-toload '2021-08-19-supervised_ImageNetBackbone_msdHeart234' --input-size 128 128 --only-last-labeled 
# --dataset 'acdc' --lr 0.01 --train-batch-size 32 --val-batch-size 8 --patience 30 --region-size 64 64 --epoch-num 1 --rl-episodes 50 --rl-buffer 600 --lr-dqn 0.001 --dqn-epochs 1 
# --budget-labels $budget --num-each-iter 16 --al-algorithm 'ralis' --rl-pool 30 --only-last-labeled

# rl-pool can't be larger than 
