...
for lr in 0.01 
    ...
    for budget in 0
        do
        $exec_command python3 -u $code_path/run.py --exp-name "2021-07-19-test_acdc_ImageNetBackbone_budget600_seed_123_lr_${lr}_budget_${budget}" \
        --seed ${SLURM_ARRAY_TASK_ID}  --checkpointer \
        --load-weights --exp-name-toload "2021-07-17-train_acdc_ImageNetBackbone_budget_608_lr_0.01_seed_123" \
        ...
        --rl-episodes 100 --rl-buffer 600 --lr-dqn 0.001 --dqn-epochs 9 --exp-name-toload-rl "2021-07-17-train_acdc_ImageNetBackbone_budget_608_lr_0.01_seed_123" \
        --budget-labels $budget --num-each-iter 0 --al-algorithm 'ralis' --rl-pool 50 \
