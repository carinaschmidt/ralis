...
for lr in 0.01
   ...
   for budget in 1536 #20% we have 7680 regions, 480 slices #3 patients #160 slices each
      do
      $exec_command python3 -u $code_path/run.py --full-res --exp-name "2021-10-31-brats18_ImageNetBackbone_stdAug_budget_${budget}_lr_${lr}_seed_${SLURM_ARRAY_TASK_ID}" \
      ...
      --lr $lr --train-batch-size 8 --val-batch-size 2 \
      --patience 30 --region-size 40 48 --epoch-num 9 \
      --rl-episodes 40 --rl-buffer 600 --lr-dqn 0.001 --dqn-epochs 9 \
      --budget-labels $budget --num-each-iter 32 --al-algorithm 'ralis' --rl-pool 30
      done
   done
