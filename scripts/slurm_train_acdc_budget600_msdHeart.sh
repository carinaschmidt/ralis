...
for lr in 0.01
   do
   for budget in 19 38 76 152 304 608
       do
       $exec_command python3 -u $code_path/run.py --exp-name "2021-07-31-train_acdc_MSDPretrained_budget_${budget}_lr_${lr}_seed_${SLURM_ARRAY_TASK_ID}_ImageNetBackbone_128128" \
...
for lr in 0.01
   ...
   for budget in 19 38 76 152 304 608
       do
       $exec_command python3 -u $code_path/run.py --exp-name "2021-07-31-train_acdc_MSDPretrained_budget_${budget}_lr_${lr}_seed_${SLURM_ARRAY_TASK_ID}_ImageNetBackbone_256256" \
       --seed ${SLURM_ARRAY_TASK_ID}  --checkpointer \
       --load-weights --exp-name-toload '2021-07-18-supervised_msdHeart0.01' \
       --ckpt-path $ckpt_path --data-path $data_path \
