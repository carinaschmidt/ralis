...
ckpt_path='/mnt/qb/baumgartner/cschmidt77_data/exp2_acdc_baselines_lr05'  #change ckpt path
...
for alalgo in 'entropy' 'bald' 'random'
   do
   for budget in 64 128 592 960 1184 1424 1904 2384 3568 #20% from 3040
      do
...
for alalgo in 'entropy' 'bald' 'random'
   ...
   for budget in 64 128 592 960 1184 1424 1904 2384 3568 
      do
      $exec_command python3 -u $code_path/calculate3Ddice_patientwise.py --checkpointer \
      --exp-name "2021-10-20-acdc_ImageNetBackbone_baseline_${alalgo}_seed${SLURM_ARRAY_TASK_ID}_budget_${budget}" \
      --data-path $data_path --ckpt-path $ckpt_path --dataset 'acdc' --al-algorithm ${alalgo}
      done
   done
