...
for lr in 0.05
   do
   for budget in 2384 3568
      do
      $exec_command python3 -u $code_path/run.py --full-res --exp-name "2021-11-08-acdc_test_RIRD_budget_${budget}_lr_${lr}_seed_${SLURM_ARRAY_TASK_ID}" \
      ...
      --rl-episodes 50 --rl-buffer 600 --lr-dqn 0.001 --dqn-epochs 9 --load-weights --exp-name-toload-rl "2021-11-05-acdc_train_RIRD_budget_608_lr_${lr}_seed_${SLURM_ARRAY_TASK_ID}" \
      --budget-labels $budget --num-each-iter 16 --al-algorithm 'ralis' --rl-pool 30 --newAugmentations --train --test --final-test
      done
   done