...
# for al_algorithm in 'ralis'
#    do
#    for budget in 64 128 592 960 1184 1424 1904 2384 3568
#        do
#        $exec_command python3 -u $code_path/run.py --exp-name "2021-11-07-acdc_${al_algorithm}_BraTSDQN_ImageNetBackbone_budget_${budget}_seed_${SLURM_ARRAY_TASK_ID}" \
#        --seed ${SLURM_ARRAY_TASK_ID}  --checkpointer \
#        --ckpt-path $ckpt_path --data-path $data_path \
#        --input-size 128 128 --only-last-labeled --dataset 'acdc' --lr 0.05 --train-batch-size 32 --val-batch-size 4 \
#        --patience 50 --region-size 64 64 --epoch-num 49 --load-weights \
#        --exp-name-toload '2021-11-03-supervised-acdc_2patients_stdAug_ImageNetBackbone_lr_0.05_234' --exp-name-toload-rl "2021-10-31-brats18_ImageNetBackbone_stdAug_budget_1536_lr_0.01_seed_55" \
...
#    for budget in 64 128 592 960 1184 1424 1904 2384 3568
#        do
#        $exec_command python3 -u $code_path/run.py --exp-name "2021-11-04-acdc_${al_algorithm}_ImageNetBackbone_budget_${budget}_seed_${SLURM_ARRAY_TASK_ID}" \
#        --seed ${SLURM_ARRAY_TASK_ID}  --checkpointer \
#        --ckpt-path $ckpt_path --data-path $data_path \
#        --input-size 128 128 --only-last-labeled --dataset 'acdc' --lr 0.05 --train-batch-size 32 --val-batch-size 4 \
#        --patience 50 --region-size 64 64 --epoch-num 49 --load-weights \
#        --exp-name-toload '2021-11-03-supervised-acdc_2patients_stdAug_ImageNetBackbone_lr_0.05_234' --exp-name-toload-rl "2021-10-26-train_acdc_ImageNetBackbone_budget_128_lr_0.05_2patients_seed123" \
#        --budget-labels $budget --num-each-iter 16 --al-algorithm $al_algorithm --rl-pool 30 --train --test --final-test
#        done
#    done
