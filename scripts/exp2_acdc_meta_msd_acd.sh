...
#       do
#       $exec_command python3 -u $code_path/run.py --full-res --exp-name "2021-10-09-acdc_train_stdAug_msdHeart_agnostic_budget_${budget}_lr_${lr}_seed_${SLURM_ARRAY_TASK_ID}" \
...
for lr in 0.01 0.05
   ...
   for budget in 608 #20% from 3040
      do
      $exec_command python3 -u $code_path/run.py --full-res --exp-name "2021-10-13-acdc_train_RIRD_msdHeart_agnostic_budget_${budget}_lr_${lr}_seed_${SLURM_ARRAY_TASK_ID}" \
      --seed ${SLURM_ARRAY_TASK_ID}  --checkpointer \
      --ckpt-path $ckpt_path --data-path $data_path \
      --input-size 128 128 --only-last-labeled --dataset 'acdc' --modality '2D' \
      --load-weights --exp-name-toload '2021-07-18-supervised_msdHeart0.01' \
