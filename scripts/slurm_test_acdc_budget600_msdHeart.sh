...
for lr in 0.01 0.05
   ...
   for budget in 16 32 64 128 960 1424 1904 2384
       do
       $exec_command python3 -u $code_path/run.py --exp-name "2021-08-01-test_acdc_MSDPretrained_ImageNetBackbone_128128_bud152_lr_${lr}_budget_${budget}" \
...
for lr in 0.01 0.05
   ...
   for budget in 16 32 64 128 960 1424 1904 2384
       do
       $exec_command python3 -u $code_path/run.py --exp-name "2021-08-01-test_acdc_MSDPretrained_ImageNetBackbone_256256_bud152_lr_${lr}_budget_${budget}" \
       --seed ${SLURM_ARRAY_TASK_ID}  --checkpointer \
       --load-weights --exp-name-toload '2021-07-18-supervised_msdHeart0.01' \
       --ckpt-path $ckpt_path --data-path $data_path \
