...
# adapted and extended by Carina Schmidt
...
def test_train_val_split(patient_id):
    ...
    #     return 'validation'
    # else:
    #     return 'test' #@carina changed from train to test

    #### 285 patients
    if patient_id % 2 == 0:
        return 'train' #50%: 143 patients
    elif patient_id % 5 == 0: #40%: 114 patients
        return 'validation'
    else: #2/6
        return 'test' #10%: 28 patients
...
def prepare_data(input_folder, output_file, size, input_channels, target_resolution):
    ...
    for folder in os.listdir(input_folder):
        print(folder)
        train_test = test_train_val_split(pid) #@carina add to train, test or val depending on pid
        pid = pid + 1
    ...
    for train_test in ['test', 'train', 'validation']:
        for folder in file_list[train_test]:
            ...
            img_c1, _, img_header = utils_acdc.load_nii(baseFilePath + "_t1.nii.gz")
            img_c2, _, _ = utils_acdc.load_nii(baseFilePath + "_t1ce.nii.gz")
            img_c3, _, _ = utils_acdc.load_nii(baseFilePath + "_t2.nii.gz")
            img_c4, _, _ = utils_acdc.load_nii(baseFilePath + "_flair.nii.gz")
    ...
    for train_test in ['train', 'test', 'validation']:
        ...
        for folder in file_list[train_test]:
            ...
            img_c3, _, _ = utils_acdc.load_nii(baseFilePath + "_t2.nii.gz")
            img_c4, _, _ = utils_acdc.load_nii(baseFilePath + "_flair.nii.gz")
            mask_dat, _, _ = utils_acdc.load_nii(baseFilePath + "_seg.nii.gz")
            ...
            ###### @carina save image and mask as .npy file #########################################
            
            data_file_name = 'pat_%s.npy' % (str(pat_id))
            #save in BraTS, volumes/labels, 'train'/'test'/'val', pat_id.npy
            data_file_path = os.path.join(preprocessing_folder, 'volumes', train_test, data_file_name) 
            # save image as array to folder        
            np.save(data_file_path, img)
            data_file_path = os.path.join(preprocessing_folder, 'gts', train_test, data_file_name)  
            # save mask as array to folder  
            np.save(data_file_path, mask)
            pat_id += 1
            ########################################################################################
...
def load_and_maybe_process_data(input_folder,
    ...
    utils_acdc.makefolder(preprocessing_folder)
...
if __name__ == '__main__':
    input_folder = "/mnt/qb/baumgartner/rawdata/BraTS2018_train/HGG_and_LGG/"
