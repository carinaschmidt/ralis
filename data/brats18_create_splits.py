...
#     d_s = np.random.choice(train, 4, replace=False) #randomly choose unique 4 pats to build D_S
#     train = np.setdiff1d(train, d_s) #removes d_s from train -> 139 left
#     d_t = np.random.choice(train, 39, replace=False) # choose 39 pats to build D_T
#     d_v = np.setdiff1d(train, d_t) #remaining 100 patients to build d_v
    
#     val_path = os.path.join(npy_vol_path, 'validation')
...
#     file_dir = os.getcwd()
#     data_file_name = 'data/brats18_pat_img_splits.npy' 
